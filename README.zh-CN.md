...
- 单灯模式一次只显示一种颜色；三灯模式会分别使用红灯、黄灯、绿灯
- 绿灯表示当前比较空闲、安静，或者任务已经完成
- 当一个任务刚结束时，绿灯会闪几下，提醒你这轮工作已经做完了
- 黄灯表示 Agent 正在忙、正在思考、需要你留意，或者在等你确认
- 黄灯持续闪烁时，通常代表现在有一条你最好立刻看一下的提醒，比如权限确认
- 红灯表示出现了异常或错误
- 如果同时有多个 Agent 在运行，状态栏会优先显示当前最需要你关注的那种情况

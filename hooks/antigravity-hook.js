...
function getAntigravityPermissionTimeoutMs(env = process.env) {
  const raw = Number(env.ai-status-beacon_ANTIGRAVITY_PERMISSION_TIMEOUT_MS);
...
function isAntigravityHookDebugEnabled(env = process.env) {
  return isTruthyDebugValue(String(env.ai-status-beacon_ANTIGRAVITY_HOOK_DEBUG || "").toLowerCase());
}

function getAntigravityHookDebugLogPath(env = process.env) {
  if (typeof env.ai-status-beacon_ANTIGRAVITY_HOOK_DEBUG_FILE === "string" && env.ai-status-beacon_ANTIGRAVITY_HOOK_DEBUG_FILE.trim()) {
    return env.ai-status-beacon_ANTIGRAVITY_HOOK_DEBUG_FILE.trim();
  }
...
function writeAntigravityHookDebug(env, event, fields = {}) {
  ...
  if (isTruthyDebugValue(String(env.ai-status-beacon_ANTIGRAVITY_HOOK_DEBUG_STDERR || "").toLowerCase())) {
    try {

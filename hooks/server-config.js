...
const CLAWD_SERVER_ID = "ai-status-beacon";
...
const RUNTIME_CONFIG_PATH = path.join(os.homedir(), ".ai-status-beacon", "runtime.json");
...
function isRemoteHookMode(options = {}) {
  ...
  const value = env && env["AI_STATUS_BEACON_REMOTE"];
...
function resolveWindowsNodeBinSync(options = {}) {
  ...
  //    In packaged Clawd builds this is `ai-status-beacon.exe`, so it falls
